...
class ContentBasedRecommender:
    ...
    def _build_feature_string(self, movie):
        ...
        if isinstance(genres_val, list):
            genres = ' '.join([str(g).strip() for g in genres_val if str(g).strip()])
        else:
            genres = str(genres_val).replace('|', ' ')
        cast = ' '.join([a.replace(' ', '_') for a in movie.get('cast', [])])
        director = movie.get('director', '').replace(' ', '_')
    ...
    def recommend(self, movie_ids, n=10, exclude_ids=None):
        ...
        scored = [(i, sim_scores[i]) for i in range(len(self.movies))
...
class CollaborativeFilteringRecommender:
        ...
        for epoch in range(self.n_epochs):
            ...
            for _, row in shuffled.iterrows():
                ...
                if u is None or i is None:
                    continue
                pred = self.global_mean + self.user_biases[u] + self.item_biases[i] + self.user_factors[u] @ self.item_factors[i]
                err = row['rating'] - pred
                total_loss += err ** 2
    ...
    def recommend_for_user(self, user_id, liked_movie_ids=None, n=10, exclude_ids=None):
        ...
        # If the user isn't in the training set (likely, since app users are in SQLite),
        # infer a pseudo-user embedding from liked movies to still enable CF-style ranking.
        if u is None:
            liked_movie_ids = [int(mid) for mid in (liked_movie_ids or []) if int(mid) in self.item_map]
            ...
            for movie_id, i in self.item_map.items():
                if int(movie_id) in exclude_ids:
                    continue
                score = self.global_mean + pseudo_bias + self.item_biases[i] + pseudo_user @ self.item_factors[i]
        ...
        for movie_id, i in self.item_map.items():
            if int(movie_id) in exclude_ids:
                continue
            score = self.global_mean + self.user_biases[u] + self.item_biases[i] + self.user_factors[u] @ self.item_factors[i]
            scored.append((int(movie_id), float(score)))
        scored.sort(key=lambda x: x[1], reverse=True)
        return [mid for mid, _ in scored[:n]]
...
class HybridRecommender:
    ...
    def recommend_new_user(self, liked_movie_ids, genres, directors, actors, exclude_ids=None, n=12):
        ...
        if liked_movie_ids:
            ids = self.content_model.recommend(liked_movie_ids, n=max(n, 30), exclude_ids=exclude_ids)
        else:
            ids = self.content_model.recommend_by_preferences(genres or [], directors or [], actors or [],
    ...
    def recommend(self, user_id, liked_movie_ids, exclude_ids=None, n=12):
        ...
        content_recs = self.content_model.recommend(liked_movie_ids, n=30, exclude_ids=exclude_ids)
        content_scores = {mid: (len(content_recs)-i)/len(content_recs) for i, mid in enumerate(content_recs)}

        # Collaborative recommendations
        collab_recs = self.collab_model.recommend_for_user(user_id, liked_movie_ids=liked_movie_ids, n=30, exclude_ids=exclude_ids)
        collab_scores = {mid: (len(collab_recs)-i)/len(collab_recs) for i, mid in enumerate(collab_recs)}
...
def get_recommender():
    ...
    if _recommender is None:
        ...
        if _recommender is None:
            ...
            ratings = [{"userId": r.user_id, "movieId": r.movie_id, "rating": r.rating} for r in load_ratings()]
